      ...
      <section class='flex flex-col gap-4'>
        <div class='relative flex flex-row justify-center items-end gap-2 w-full max-w-lg mx-auto h-48 sm:h-56 transition-all duration-500'>
          {podium.map((comp, i) => {
            ...
            const baseColors = [
              `bg-${settings.theme.accentColor}-400`, // 1st
              `bg-${settings.theme.accentColor}-500`, // 2nd
              `bg-${settings.theme.accentColor}-600`, // 3rd
            ]
            const order = [2, 3, 1][i]
            return (
              <div
        ...
        <Ul class='ml-0! list-none'>
          {rest.map(comp => (
            <li key={comp.name} class='mb-2'>
              <Card
                as='article'
                class='flex items-center flex-row! gap-4 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-200 dark:border-gray-700'
              >
                <header
                  class={`rounded-full w-8 h-8 flex items-center justify-center bg-${settings.theme.accentColor}-300 dark:bg-${settings.theme.accentColor}-500`}
                >
                  <span class='font-bold text-white text-xl'>{comp.place}</span>
                </header>
